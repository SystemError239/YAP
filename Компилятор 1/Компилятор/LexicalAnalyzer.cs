...
namespace Компилятор
{
    class LexicalAnalyzer
    {
        ...
        public byte NextSym()
        {
            ...
            switch (InputOutput.Ch) //посимвольный выбор 
            {
                case char c when char.IsDigit(InputOutput.Ch): //целые числа 
                    if (((InputOutput.Ch >= '0' && InputOutput.Ch <= '9'))) //промещуток 
                    {
                        ...
                        symbol = intc;//Символ  = целая константа
                        return symbol;
                    }
                    break;
                case char c when char.IsLetter(InputOutput.Ch): //раздел юникода с буквами 
                    ...
                    endworld = false;
                    while ((((InputOutput.Ch >= 'a' && InputOutput.Ch <= 'z') || (InputOutput.Ch >= 'A' && InputOutput.Ch <= 'Z')) || (InputOutput.Ch >= '0' && InputOutput.Ch <= '9')) || ((InputOutput.Ch >= 'А' && InputOutput.Ch <= 'Я') || (InputOutput.Ch >= 'а' && InputOutput.Ch <= 'я'))) //пока все это дело находится 
                    {
            ...
            return symbol;//Возвращаем код символа
        }
        public byte PeekNextSym()
        {
            long oldPos = InputOutput.File.BaseStream.Position;
            int oldChar = InputOutput.File.Peek(); // посмотреть символ
            if (oldChar == -1) return 0;

            // вернуть символ, как будто он был NextSym
            return (byte)oldChar;
        }
